    ...
    return (
        <div className="m-4 w-9/12 mx-auto">
            ...
            <div className="justify-center text-center">
                ...
                <h2 className="font-bold  text-sm m-1 p-4 text-left">{cuisines.join(" , ")}</h2>
            </div>
            ...
            <div className="text-center m-4 p-4 bg-gray-100">
                {categories.map((item,index)=><CategoriesMenu key={item.id} itemx={item} showItems={index==showIndex ? true: false} setShowIndex ={()=>setShowIndex(index)}/>)}
            </div>
        </div>
    )
}
